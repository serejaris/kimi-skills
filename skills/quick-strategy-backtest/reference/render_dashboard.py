...
def _load_trades_csv(path: str | Path) -> list[dict[str, Any]]:
    ...
    with open(path, encoding="utf-8") as f:
        ...
        for row in reader:
            for k in (
                "size", "entry_price", "exit_price", "pnl", "pnl_pct",
...
def _normalize_trade_history_display(
    ...
    # Sort earliest entry_date first so the trades table reads chronologically.
    # Under lot-level FIFO, write order can be close-time order (partial sells
    # close older lots first), which confuses readers expecting entry-time
    # order. Missing entry_date rows fall to the bottom; stable sort preserves
    # relative order otherwise.
    normalized.sort(
        key=lambda row: (
            row.get("entry_date") in (None, ""),
...
def render_dashboard(
    ...
    title = (
        report_data.get("meta", {}).get("strategy_name")
        or _resolve_locale(dashboard_language)["default_title"]
    )
...
def _build_default_modules(
    ...
    modules = [
        ...
        },
        {
            ...
            "subtitle": f"{meta.get('strategy_name') or L['default_strategy_name']} · {L['metric_table_subtitle_suffix']}",
...
def _build_event_modules(
    ...
    if event_overview_mode in {"timeline", "both"} and equity_curve:
        modules.append({
            ...
            # Event-study equity_curve is a normalized index anchored at 100, so
            # "absolute" mode would just show "100 + percentage" — no extra info,
            # only confusion (looks like dollars). Hide the toggle entirely.
            "modes": [],
            "hide_value_row": True,
