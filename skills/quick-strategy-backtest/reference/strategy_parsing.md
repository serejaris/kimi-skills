...
| Allow scaling in | No (default flat-position gate) / Yes (prompt has add-on / scale in / every signal / grid / staged entry / overlapping events) | ✅ / ⚠️ |
| Position accounting | lot_level (default) / aggregate_average_cost (narrow opt-in) | ✅ / ⚠️ |
| Lot matching rule | fifo (default `close_lots_fifo`) / custom loop (LIFO / sell-the-latest-add-on / grid per-level — write your own using `close_lot`) | ✅ / ⚠️ |

Defaults: `position_accounting="lot_level"` + flat-position entry gate + full-exit. If the prompt mentions add-ons / averaging down / grid / staged entry / sell half / reduction / target weight / rotation / rebalance / overlapping events, fill the "Allow scaling in" and "Lot matching rule" rows accordingly; everything else stays default. Switch to `aggregate_average_cost` only when the user explicitly asks for "per-trade PnL on average cost" / "alignment with broker statement" / "pure DCA".
