...
> Universal rules → See `SKILL.md` §Core Principles.
> Task-specific rules below:
- ✅ Final PDF **≥25 pages**, each major module ≥1.5-2 pages
- ✅ **L2**: ALL numbers from Task 2 Excel via openpyxl; ≥10 cross-checks
- ✅ **L1**: Numbers from Task 1 Research Document; comps-based valuation
- ✅ ≥15 exhibits (L2) or ≥10 (L1), including C6 stock chart from real API data
- ✅ **Page 2 = Data Summary page** (mandatory 2-column dense summary)
- ✅ Active web research per module to enrich content to match the word count requirements.
- ✅ Word count should match the requirements in `output/report-layout.md`
- ✅ References & Data Sources section with URLs; Glossary with 6-12 terms
- ✅ Chart-count check: `python scripts/embed_charts.py count --html report.html` ≥3
- ❌ Do not say "see financial model for details" — EXTRACT the details
- ❌ Do not skip any module — ALL mandatory (L1: 18 modules, L2: 21 modules)
...
- ❌ **DO NOT call or invoke the environment PDF skill.** Paged.js has strict Mermaid limits (≤12 nodes) that break supply chain diagrams.
- ❌ **DO NOT let any tool generate a cover page.** Our report has its own cover page design in `output/report-layout.md`.
- ❌ **DO NOT add headers, footers, or front matter** not specified in our layout file. Use Playwright `header_template`/`footer_template` in `page.pdf()`.

**Code** (from `output/report-layout.md` §4.9):
```python
...
# margin=0 → let CSS @page { margin: 18mm 20mm } control margins
```
...
| 3   | `output/report-layout.md`         | HTML structure, module order, word count required for each module, CSS, L1/L2 layout differences | ✅   | ✅   |
...
html_with_css = ''.join(html_parts)

# Step 2: Generate PDF via Playwright (sole method - do NOT use WeasyPrint or PDF skill)
page.set_viewport_size({"width": 1200, "height": 1600})
page.emulate_media(media="print")
page.set_content(html_with_css, wait_until="networkidle")
...
header_html = (
    ...
    '<span style="white-space:nowrap;">COMPANY (TICKER)</span></div>'
)
footer_html = (
    '<div style="display:flex;justify-content:space-between;width:100%;'
...
| 5 | **Research Document (.md)** | ≥9,000 words, all sections complete | Task 1 output |
