...
description: "Task 1 of equity report workflow. Produces a Research Document (≥9,000 words) consumed by Task 2 and Task 3. Entry point after SKILL.md routes output_type = EQUITY_REPORT."
---

# Equity Report — Task 1: Research + Analysis

> **Mode**: Equity Report (3-Task Architecture)
> **This is Task 1 of 3.** It produces a Research Document (≥9,000 words) consumed by Task 2 and Task 3.
...
> Universal rules (data authenticity, no fabrication, no placeholders) → See `SKILL.md` §Core Principles.
> Task-specific rules below:
- ✅ Write the full Research Document — ≥13,500 words, every section complete (§I ≥600w, §II ≥2,250w, §III ≥1,800w, §IV ≥2,250w)
- ✅ §VI Historical Financials: Fill ALL cells for ≥3 years (no blanks)
...
- ✅ Phase 2.7 deep research: Complete ALL 6 sub-steps
- ❌ Do not abbreviate sections to save context — this document feeds Task 2 and Task 3
- ❌ Do not skip Phase 2.7 or merge sub-steps
...
**Language Selection (Deterministic Algorithm)**:

Language is NOT a judgment call. Follow this exact algorithm:

```
Step 1: Detect user query language
  ├─ Count Chinese characters vs ASCII letters in user's message
  ├─ ≥50% Chinese characters → query_lang = "zh"
  ├─ ≥50% ASCII/Latin → query_lang = "en"
  └─ Truly mixed (e.g., "AAPL怎么样") → query_lang = "zh" (Chinese context dominates)

Step 2: Determine market "natural language"
  ├─ .SH / .SZ / .BJ / .HK → market_lang = "zh"
  └─ .US or no suffix (US stock) → market_lang = "en"

Step 3: Final determination (NO exceptions)
  ├─ query_lang == market_lang → report_language = query_lang (use directly, no ask)
  └─ query_lang != market_lang → MUST ask user: "Would you prefer the report in Chinese or English?"
       └─ ⛔ Stop and WAIT for user response. Do NOT proceed without explicit answer.

Step 4: Lock
  └─ Once set, report_language NEVER changes without user confirmation
```
...
| 2.7.6 | `analysis/risk-framework.md` | 8-12 categorized risks with probability × impact scoring | 1,800-2,700 |
...
### 3.4 Output Document: Research Document — Word Count Validation

Before writing the final document, validate word counts. Run this inline check:

```python
# Inline word count validation (soft gate — warn but do not block delivery)
min_counts = {
    "Total Document": 13500,
    "Section I (Core Narrative)": 600,
    "Section II (Six-Dimension)": 2250,
    "Section III (Company Overview)": 1800,
    "Section IV (Industry & Competition)": 2250,
}
# Count words per section and compare to min_counts
# For any section below minimum, EXPAND it before delivery
# For total below 9000, add more analytical depth
```

**Action**: If any section is below minimum, expand analysis with additional data points and 
"so what" reasoning. Do NOT deliver a thin document — it produces a thin report in Task 3.
