...
- ✅ Build a REAL Excel model with formulas — not static numbers
- ✅ All 3 financial statements (IS → BS → CF) must link and balance
- ✅ BS: Assets = L+E every year (BALANCE CHECK row mandatory)
- ✅ CF: Ending Cash ties to BS Cash every year (CASH TIE-OUT row mandatory)
- ✅ Revenue links from Revenue Model tab → IS tab (not typed twice)
- ✅ DCF: WACC from CAPM, FCF discounted with formulas
- ✅ Sensitivity: Excel formulas referencing WACC + Terminal Growth inputs
- ✅ Comps: ≥5 peers + statistical summary (Max/75th/Median/25th/Min)
- ❌ Do not hardcode projected numbers
- ❌ Do not skip Scenarios tab
- ❌ Do not fabricate comparable company data
