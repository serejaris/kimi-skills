...
⚠️ **PDF renderers (Playwright `page.pdf()`, Chromium print engine) do NOT execute client-side JavaScript.**
...
**Rendering via Playwright**: Use the browser automation toolkit to render Mermaid code to a PNG:
1. Write Mermaid code to a temporary HTML file with `<script src="https://cdn.jsdelivr.net/npm/mermaid@10/dist/mermaid.min.js">`
2. Call `browser_visit` on the file, then `browser_screenshot`
3. Convert the PNG to base64 and embed as `<img src="data:image/png;base64,...">`
4. Wait ≥3 seconds for CDN load + render; retry once if timeout
5. If Playwright fails after 1 retry → use HTML/CSS flex fallback immediately

**Key parameters**: `viewport={'width': 1200, 'height': 800}`, `theme: 'base'`, 
`flowchart: {curve: 'basis', padding: 6, nodeSpacing: 30, rankSpacing: 70}`.
python
...
- No raw `<pre class="mermaid">` in PDF-destined HTML (PDF renderers don't execute JS)
- No `<script src="mermaid.min.js">` in final HTML
- No pure `<table>` or pure text replacing the value chain diagram
- Use `.chart-container-free` (NOT `.chart-container` with 170px limit)
...
### Layout Optimization: Control SVG Height

Large Mermaid SVGs exceeding A4 page height can break PDF pagination. Optimize layout for same content with less height:
...
**SVG height detection**: After render complete, detect SVG height; when exceeding 900px (A4 available height), apply `transform: scale()` shrink via CSS.
...
□ ≥4 value chain levels identified □ Real company names per level □ Target company nodes highlighted
□ Mermaid flowchart LR with subgraphs □ Playwright render with ≥3s wait □ HTML/CSS flex fallback ready


### Post-Render Check
□ Chart displays fully, no truncation □ Node text readable □ Colors correct
□ Target company highlighted □ No raw `<pre class="mermaid">` in HTML
