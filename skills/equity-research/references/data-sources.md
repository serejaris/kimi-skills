...
### iFind API Parameters

```python
ifind_get_stock_info(ticker: str, file_path: str)
ifind_get_financial_statements(ticker: str, statement: str, financial_parameter: str, file_path: str)
ifind_get_stock_financial_index(ticker: str, financial_parameter: str, file_path: str)
ifind_get_stock_business_segmentation(ticker: str, financial_parameter: str, file_path: str)
ifind_get_forecast(ticker: str, file_path: str)  # A-shares only, single ticker
ifind_get_price(ticker: str, start_date: str, end_date: str, file_path: str, adjust: str = "forward")
ifind_get_stock_announcement(ticker: str, start_date: str, end_date: str, file_path: str)
```

**financial_parameter format**: `YYYYMMDD` where MM DD ∈ {0331, 0630, 0930, 1231}
- `"20241231"` = FY2024 annual
- `"20250331"` = Q1 2025
- **Wrong formats**: `"2024"`, `"FY2024"`, `"2024Q1"` → will fail

**ifind_get_price 5-parameter checklist**:
| Parameter | Correct Format | Common Errors |
|-----------|---------------|---------------|
| `ticker` | A股: `XXXXXX.SH/SZ/BJ`; HK: `XXXX.HK`; US NYSE: `XXX.N`; US NASDAQ: `XXX.O` | Missing suffix; wrong suffix |
| `start_date` / `end_date` | `YYYYMMDD` | `YYYY-MM-DD`, Unix timestamp |
| `adjust` | `"forward"` (exact string) | `"fwd"`, `"F"`, `"Forward"` |
| `file_path` | Valid writable path ending in `.csv` | Non-writable dir, missing extension |
| Cross-market | Do NOT mix A股 + HK/US in same call | Returns empty for all tickers |
...
get_historical_stock_prices(ticker: str, period: str, interval: str)
```
...
### Stock Code Format Reference

| Market | iFind Format | Yahoo Format |
|--------|-------------|--------------|
| A股 Shanghai | `XXXXXX.SH` | `600519.SS` |
| A股 Shenzhen | `XXXXXX.SZ` | `XXXXXX.SZ` |
| HK stocks | `XXXX.HK` | `XXXX.HK` |
| US NYSE | `XXX.N` | `XOM` (no suffix) |
| US NASDAQ | `XXX.O` | `AAPL` (no suffix) |

> **⚠️ Cross-market restriction**: Do NOT mix A-shares with HK/US stocks in iFind `business_segmentation` or `related_stock` calls.
