...
This page constructs the OAuth URL from environment variables and redirects the user. The route `/login` should point to this component in `App.tsx`. Do not modify the OAuth URL construction logic or create a custom login page. The supported scope is `profile`.

There is no separate registration flow: users are auto-provisioned (upserted) on first OAuth login. Register/sign-up CTAs link to `LOGIN_PATH` (from `@/const`).
