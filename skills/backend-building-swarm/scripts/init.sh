...
# ── Backend Building Init (Swarm, Feature-Based) ───────────────
# Grafts backend infrastructure (tRPC + Drizzle + Hono) onto an
# existing webapp-building frontend at $PROJECT_PATH. Supports
# incremental feature installation via --features, and provision-only
# --template mode. Tracks installed features in .backend-features.json.
#
# Operates IN PLACE on $PROJECT_PATH and commits the graft on whatever
# branch is currently checked out there. It does NOT manage git
# worktrees or merge branches — that is the orchestrator's job:
#   - Swarm: the main agent creates a `backend` worktree via
#     swarm-workspace's setup-local.sh, runs this itself with
#     PROJECT_PATH=$HOME/app-backend, then merges the `backend` branch
#     into the integration line. It runs the graft itself rather than via
#     a separate subagent because the `.env` written here is gitignored:
#     it never travels through the merge, so it stays only in the sandbox
#     where this ran. The main agent's sandbox is the one that later
#     builds; a separate subagent's `.env` would be stranded.
#   - Single-agent / tests: PROJECT_PATH defaults to
#     /mnt/agents/output/app — same behaviour as backend-building-v2.
# ─────────────────────────────────────────────────────────────────
...
# Use -e (any kind of entry) instead of -d so a worktree's .git file is detected.
...
install_auth() {
  ...
  # The auth UI (Login page, useAuth hook, AuthLayout) is graft-owned: this is
  # the one authoritative source, so plant it with `cp` (overwriting any
  # scaffold placeholder at the same path). NotFound is shared territory, so it
  # keeps `cp -n` and yields to an existing page.
  ...
  cp "$TMPL/auth/client-patches/pages/Login.tsx"    "$PROJECT_PATH/src/pages/Login.tsx"
  ...
  cp "$TMPL/auth/client-patches/components/AuthLayout.tsx" \
    "$PROJECT_PATH/src/components/AuthLayout.tsx"
  cp "$TMPL/auth/client-patches/components/AuthLayoutSkeleton.tsx" \
...
if [[ ${#NEW_FEATURES[@]} -gt 0 ]]; then
  ...
  APP_INFO=$(curl -sf --connect-timeout 5 --max-time 15 -X POST "$PORTAL_URL" \
    -H "Content-Type: application/json" \
    -d "{\"name\":\"$APP_TITLE\",\"features\":$FEATURES_JSON}") || {
    echo "Error: Failed to reach portal at $PORTAL_URL (timed out or unreachable)"
    exit 1
  }

  # Guard: portal must return JSON — a 502/proxy error would be an HTML page,
  # which would otherwise blow up the parses below with a cryptic SyntaxError.
  echo "$APP_INFO" | node -e "try { JSON.parse(require('fs').readFileSync('/dev/stdin','utf8')); } catch (e) { console.error('Error: Portal returned a non-JSON response (it may be down or returned an error page)'); process.exit(1); }" || exit 1
...
echo ""
echo "Committed the graft on branch '$(git -C "$PROJECT_PATH" rev-parse --abbrev-ref HEAD)' in $PROJECT_PATH."
echo "Swarm: the main agent merges this branch into the integration line."
echo ""
