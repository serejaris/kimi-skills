...
| Problem | Typical risk | Default correct pattern | Delivery check |
|---|---|---|---|
| Look-ahead bias | The decision uses information unavailable at that time | Generate the signal on `bar i`, fill at `bar i+1 open` | `common_pitfalls: 2 / 3` |
| Event vs state | A persistent state is mistaken for a one-time event and fires every bar | Use crossover / edge-trigger logic for point-in-time events | `common_pitfalls: 1 / 3` |
| Stop-loss / take-profit | `close` misses intraday touches, or same-bar ordering is assumed in your favor | Use `high/low` checks and disclose same-bar ordering assumptions | `common_pitfalls: 1 / 3` |
| Trade recording | Trades are recorded when the signal fires, so date/price/PnL are wrong | Append `trade_history` only after actual execution | `common_pitfalls: 1 / 3` |
| Position sizing and fees | `size=0` silently drops trades, or PnL ignores fees | Guard `size > 0` and deduct fees on both entry and exit | `common_pitfalls: 4` |
| Scaling / partial exits | `entry_price` / `entry_date` are overwritten and multiple lots collapse into one trade | Use lot-level or average-cost accounting explicitly | `common_pitfalls: 4b` |
| Open positions at the end | `equity_curve` contains floating PnL but `trades.csv` has no matching close | Forced close, exclusion, or mixed disclosure must be explicit | `common_pitfalls: 1 / 5` |
| Warmup segment | Indicators are not ready, or warmup bars open positions before evaluation | Load earlier data, gate the evaluation start, and slice export metrics | `common_pitfalls: 5` |

---

## Look-Ahead Bias

This is the most fatal bug in backtesting. The test is not a fixed API name; it is whether the current decision uses information unavailable at that time. Signals, selection, sizing, stop-loss, and take-profit logic may only use data from the current bar and earlier that is known at the decision time. The next bar's open may be used as the execution price after a bar-i signal is confirmed, but it must not feed back into the bar-i decision. Event studies may use future-N-day returns as outcome variables, but not to select events.

**Assessment Questions (mandatory)**

For every **order / selection / rebalance / stop-loss / take-profit / sizing** decision point, walk through the 4 questions below. Any Q that cannot be answered "safe" is a hit and must be explained or fixed. The 4 questions correspond to the 4 underlying mechanisms of information leakage — apply them to new patterns the list does not name.

- **Q1 — Index**: is every value used on this line indexed at or before the current bar?
  Example: `shift(-N)` reads a future value
- **Q2 — Window**: does this statistic / aggregate's window include any future bar?
  Example: `rolling(..., center=True)` borrows half its window from the future
- **Q3 — Cross-section / Universe**: does this ranking / selection require the full sample to finish, and is the universe itself a survivor set?
  Example: backtesting today's S&P 500 constituents ten years ago bakes in survival
- **Q4 — Data visibility**: was this field / price actually visible on the decision date?
  Example: joining fundamentals on `report_date` while the data is not yet public during the report period

Each Q carries only one canonical example. **It is not a checklist.** The Q itself is what you must answer — `qcut` on the full sample, `groupby().transform('mean')` over the whole span, `StandardScaler.fit(full_series)`, surviving-universe selection, forward-adjusted prices that absorb future dividends, `bfill` propagating future values, and similar patterns are different Qs of the same mechanism. For unfamiliar APIs (hand-rolled sliding windows, third-party factor libraries, etc.), apply the mechanism — do not rely on "I did not see it in the examples, so it must be safe".

**Appendix: high-risk explicit patterns (grep — for fast triage, not the full boundary)**
...
Grep keywords: `shift(-` / `iloc[i+` / `bfill` / full-sample `mean / quantile / max / min`. A clean grep does not mean Q1-Q4 are all answered — the appendix only helps you **catch the obvious cases first** and never replaces the manual walk.
...
For continuous daily-bar strategies, start from a pending-signal structure and declare `SIGNAL_TIMING`, `EXECUTION_TIMING`, and `EXECUTION_NOTE` at file top.

**Extra check: signal availability time**. Every trade must satisfy `signal_time <= order_time <= fill_time`. For example, `close > ma20`, full-day volume expansion, and daily high/low triggers are not known at the same bar's open, so they cannot justify same-bar-open execution. If same-day-close execution is used, disclose it as a close-confirmed / close-fill assumption. If daily bars use the same bar's high for entry and low for stop-loss, the intrabar order is unknown; do not assume the favorable sequence by default.
...
## Scaling In / Out Must Not Overwrite Entry State

For add-ons, staged buys, grids, partial take-profit / stop-loss, do not maintain only one `entry_date / entry_price / position`.

```python
# Wrong: the second add-on overwrites entry_price, or total position is attached
# to the first entry_date.
position += add_size
entry_price = price
trade_history.append({"entry_date": entry_date, "size": position, ...})
```

Default to lot-level accounting: each buy creates a lot, sells match lots by FIFO, and each closed lot or lot slice writes one closed-trade row.
...
open_lots.append({
    ...
    "size": size,
    "entry_bar": i,
    "entry_fee": size * price * buy_commission,
})
...
while remaining > 0 and open_lots:
    ...
    if t_plus_1 and i <= lot["entry_bar"]:
        break
    proceeds = close_size * sell_price * (1 - sell_commission - sell_tax)
    buy_cost = close_size * lot["entry_price"]
    buy_fee = lot["entry_fee"] * close_size / lot["size"]
    ...
    lot["size"] -= close_size
    lot["entry_fee"] -= buy_fee
    remaining -= close_size
    if lot["size"] <= 0:
        open_lots.pop(0)
```

If the user explicitly wants multiple add-ons merged into one trade, maintain weighted-average cost and disclose that `entry_date` is only a convention and win rate is counted by aggregated close rows.
